...
RUN curl -LO https://github.com/dandavison/delta/releases/download/0.8.3/git-delta_0.8.3_amd64.deb \
    && dpkg -i git-delta_0.8.3_amd64.deb
RUN apt-get update && apt-get install -qq -y --no-install-recommends neovim fzf bat
# set unicode
ENV LANG C.UTF-8
ENV LC_ALL C.UTF-8
